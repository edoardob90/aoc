...
@click.command()
@click.argument('min', default=0.0)
@click.argument('max', default=1.0)
@click.argument('current', default=0.0)
@click.argument('width', default=10)
@click.option('--style', default=6, help='Bar style')
def progress(min, max, current, width, style, before='|', after='|'):
    """
    ...
    print(before + ''.join([
        ...
        for x
        in [y * len(style) for y in range(1, width + 1)]
    ]) + after)
